...
\begin{theorem}[Characterisation of kernels]    
    A function $k:\mathcal{X} \times \mathcal{X} \to \mathbb{R}$ which is either continous or has finite domain can be decomposed as $k(x,y)=\langle \phi(x), \phi(y) \rangle_{\mathcal{H}}$ if and only if $k$ satisfy the positive semidefinite property.
\end{theorem}

That is the function $k$ can be written as the inner product in the Hilbert space $\mathcal{H}$ of the features evaluated at the arguments $\phi(x)$.
...
\\
$\mathcal{F} =\{ \sum\limits_{i=1}^n \alpha_i k(x_i, \cdot): n \in \mathbb{N}, x_i, y_i \in \mathcal{X}, a_i \in \mathbb{R}, i=1,\cdots, n\}$.
...
Moreover, $\langle f, g \rangle_{\mathcal{F}}$ is real valued, symmetric and bilinear. All these facts imply that the just introduced inner product is a valid metric.
\\
...
\end{equation}
\\
We have then, that $f_n(x)$ is a Cauchy sequence of real numbers. Thus, we can let $g(x):=\underset{n\to \infty}\lim f_n(x)$ and include all such limits functions in $\mathcal{F}$, doing so we complete $\mathcal{F}$ and we obtain the Hilbert space $\mathcal{H}$ associated to the kernel $k$.
...
\begin{theorem}
    [Riesz Representation]. If $L : \mathcal{H} \rightarrow \mathbb{R}$ is a bounded linear operator on a Hilbert space $\mathcal{H}$ , there exists some $h_0 \in \mathcal{H}$ such that $L(f) = \langle f,h_0\rangle_\mathcal{H}, \ \forall f \in \mathcal{H}$.
\end{theorem}
...
\begin{definition}
    Considering a set of vectors $x_1, \cdots, x_n$, the Gram matrix is defined as the $n\times n$ matrix whose entries are $\langle x_i, x_j \rangle_{\mathcal{X}}$.    
\end{definition}
...
\begin{table}
    ...
    \begin{tabular}{lll}
       ...
       Gaussian RBF &  $k(x,y)=\exp\left(-\frac{\|x-y|\|_{2}^2}{2\sigma^2}\right)$ &   $\sigma$\\
       Exponential RBF/Laplacian& $k(x,y)=\exp\left(-\frac{\|x-y\|_{1}}{\gamma}\right)$ &   $\gamma$\\
       ...
       Chi-squared kernel &  $k(x,y)=\exp\left(-\gamma \sum\limits_i \frac{(x_i - y_i)^2}{x_i+y_i}\right) $ &   $\gamma$\\
        Cosine  &  $k(x,y)=\frac{xy^\intercal}{\|x\|_{2}\|y\|_{2}}$ &   \\
