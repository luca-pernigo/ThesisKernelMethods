...
Using the link between RKHS and feature spaces, we can rewrite $f(x)=\langle w, \phi(x) \rangle+b$. Moreover, note that the RKHS norm is defined as follows $\|f\|_{\varphi }=\inf\{\|w\|_{F}:w\in F,f(x)=\langle w,\varphi (x)\rangle _{F},\forall x\in X\}$, where $F$ is the feature space.
...
\\
Constraint qualifications are condtions under which strong duality holds; one the most popular is Slater's condition.
...
\\
In our case we don't have equlity constraint, so we do not have the $h_i(x)=0$ \ref{slater_condition} to be checked. All we have to check is the convexity of our problem and that there exist a $x$ such that $r_i(x)<0$.
For convexity, a sufficient condition is the positive definiteness of $Q$ in the quadratic programming problem 
\begin{equation}
    \begin{aligned}
        \min & x^\intercal Q x+ c^\intercal x \\        
        s.t & Ax\preceq b
    \end{aligned}
\end{equation}
...
Next we check Slater's condition holds. Considering first the non negative constraints on $\xi$ and $\xi^*$, we conclude that $\xi$  and $\xi^*$ have to be greater or equal to zero for the existence of an $x$ satisfying Slater's condition. Thus, let us suppose that $0 \leq \xi \leq \alpha$ and $0 \leq \xi^* \leq \alpha$.
\\
...
\begin{equation}\label{eq:kqr_min6}
    ...
    &\alpha^\intercal\mathbb{1}
    \end{aligned}
    \end{equation}
Switching sign we rephrase as a minisation problem, which is the common practice in convex optimisation.
...
\begin{equation}
    ...
    \textrm{with} \ b_i=& y_i-\sum\limits_i \alpha_i k(x_i, x_i) \\
