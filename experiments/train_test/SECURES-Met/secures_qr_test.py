...
train=pd.read_csv(f"/Users/luca/Desktop/ThesisKernelMethods/experiments/Data/SECURES-Met/{country}/clean/train/df.csv")
...
for i,q in enumerate(quantiles):
    # load model
    krn_q=pickle.load(open(f'/Users/luca/Desktop/ThesisKernelMethods/experiments/train_test/SECURES-Met/{country}/{ktype}/krn_qr_{q}.pkl', 'rb'))
    # predict
    y_predict_q=krn_q.predict(X_test_scaled)
    print(mean_pinball_loss(y_test,y_predict_q, alpha=q))
    # put in predict df
    df_predict[f"{q}"]=pd.Series(y_predict_q)

# # save predictions to csv
df_predict.to_csv(f"/Users/luca/Desktop/ThesisKernelMethods/experiments/Data/SECURES-Met/{country}/clean/model_prediction_{ktype}.csv", index=False)
