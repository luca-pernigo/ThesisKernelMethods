...
# random_state=0,
    ...
    for i,q in enumerate(tqdm(quantiles)):
        ...
        pickle.dump(qr_krn_models[i], open(f'experiments/train_test/Price/{ktype}/task {ith}/krn_qr_{i}.pkl', 'wb'))
        
# cv results
df_cv_res=pd.DataFrame(cv.cv_results_)
df_cv_res.to_csv(f"experiments/train_test/Price/{ktype}/task {ith}/models_{ktype}_gridsearch.csv",index=False)
