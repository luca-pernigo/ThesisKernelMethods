...
def test(ith):
    df=pd.read_csv(f"experiments/Data/Load/Task {ith}/L{ith}-test_clean.csv")
    ...
    df_train=pd.read_csv("experiments/Data/Load/L-train.csv")
    ...
    for i,q in enumerate(quantiles):
        krn_q=pickle.load(open(f"experiments/train_test/Load/{ktype}/task {ith}/krn_qr_{i}.pkl", "rb"))
    ...
    # reo.to_csv(f"experiments/Data/Load/Task {ith}/L{ith}-model_prediction_{ktype}.csv", index=False)
