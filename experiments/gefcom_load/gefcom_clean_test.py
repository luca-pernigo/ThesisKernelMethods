...
def clean_test(ith):
    if ith==15:
        ...
        df["DS"]=pd.to_datetime(df[["YEAR","MONTH","DAY","HOUR"]] , format='%Y%m%d')


        df=df[["LOAD","DAY","MONTH","HOUR","DAY_OF_WEEK","IS_HOLIDAY","w_avg","DS"]]
        
        # save to csv
        df.to_csv(f"/Users/luca/Desktop/GEFCom2014 Data/Load/Task {ith}/L{ith}-test_clean.csv", index=False)
